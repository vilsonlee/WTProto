...
- (instancetype)initWithName:(const char *)name concurrent:(bool)concurrent
{
    ...
    if (self != nil)
    {
        ...
        _queue = dispatch_queue_create(_name, DISPATCH_QUEUE_SERIAL);
        if (concurrent)
            _queue = dispatch_queue_create(_name, DISPATCH_QUEUE_CONCURRENT);
